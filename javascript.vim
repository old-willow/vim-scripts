...
" jQuery
  syntax keyword jQueryEventsKeyWords     bind blur change click dblclick delegate die error currentTarget data isDefaultPrevented isImmediatePropagationStopped isPropagationStopped namespace pageX pageY preventDefault relatedTarget result stopImmediatePropagation stopPropagation target timeStamp type which focus focusin focusout hover keydown keypress keyup live load mousedown mouseenter mouseleave mousemove mouseout mousehover mouseup off on one $.proxy ready resize scroll select submit toggle trigger triggerHandler unbind undelegate unload
  syntax keyword jQueryEffectsKeyWords    animate clearQueue delay dequeue fadeIn fadeTo fadeToggle finish hide queue show slideDown slideToggle slideUp stop toggle
  syntax keyword jQueryCssKeyWords        addClass after append appendTo attr before clone css detach empty hasClass height html innerHeight innerWidth inserAfter insertBefore offset offsetParent outerHeight outerWidth position prepend prependTo prop remove removeAttr removeClass removeProp replaceAll replaceWidth scrollLeft scrollTop text val width wrap wrapAll wrapInner
  syntax keyword jQueryTraversingKeyWords add addBack andSelf children closest contents each end eq filter find first has is last map next nextAll nextUntil not offsetParent parent parents parentsUntil prev prevAll prevUntil siblings slice
  syntax keyword jQueryAjaxKeyWords       ajax ajaxPrefilter ajaxSetup ajaxTransport get getJSON getScript param post ajaxComplete ajaxError ajaxSend ajaxStart ajaxStop ajaxSuccess load serialize serializeArray
  syntax keyword jQueryMiscKeyWords       data each get index noConflict param removeData size toArray
  syntax keyword jQueryPropertiesKeyWords context jquery jQuery jQuery.fx.interval jQuery.fx.off jQuery.support length
...
if version >= 508 || !exists("did_javascript_syn_inits")
  ...
  HiLink javaScriptStringS              Character
  HiLink javaScriptStringD              String
  HiLink javaScriptRegexpString         Character
  "HiLink javaScriptRegexpString         javaScriptRegexp
  ...
  " Ajax Highlighting
    ...
    "" gandalf: added Jan 2, 2013.
    HiLink javaScriptEventListenerMethods Keyword

    HiLink javaScriptNumber            Number
    HiLink javaScriptPropietaryObjects Constant

    "" gandalf added for jQuery Aug 27, 2015.
    HiLink jQueryEventsKeyWords        jQueryGeneral
    HiLink jQueryEffectsKeyWords       jQueryGeneral
    HiLink jQueryCssKeyWords           jQueryGeneral
    HiLink jQueryTraversingKeyWords    jQueryGeneral
    HiLink jQueryAjaxKeyWords          jQueryGeneral
    HiLink jQueryMiscKeyWords          jQueryGeneral
    HiLink jQueryPropertiesKeyWords    jQueryGeneral
